...
<head>
    ...
    <title>twzif | address_edit</title>
</head>
...
<form action="{{route("address_edit_function")}}" method="post" style="padding:30px;position:relative;top:200px;left:50%;transform:translateX(-50%);display:flex;justify-content:center;align-items:center;flex-direction:column;border:2px solid #001e4c;width:50%;border-radius:5px">
    @csrf
    <div class="input" style="border:2px solid #001e4c;border-radius:5px">
    <label for="address" style="padding:10px 30px 10px 5px">address</label>
    <input type="text" value="{{old('address')}}" name="address" id="address" placeholder="country,city,street" style="padding:10px 70px 10px 5px;border:0;outline:0;">
    </div>
    @error("address")
