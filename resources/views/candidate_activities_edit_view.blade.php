...
<head>
    ...
    <title>twzif | activities edit</title>
</head>
...
<form action="{{route("candidate_activities_edit_function")}}" method="post" style="padding:30px;margin-top:200px;position:relative;left:50%;transform:translateX(-50%);display:flex;justify-content:center;align-items:center;flex-direction:column;border:2px solid #001e4c;width:50%;border-radius:5px">
    @csrf
    <div class="input" style="border:2px solid #001e4c;border-radius:5px">
    <label for="activity" style="padding:10px 30px 10px 5px">add activity</label>
    <input type="text" value="{{old('activity')}}" name="activity" id="activity" placeholder="Enter your activity" style="padding:10px 70px 10px 5px;border:0;outline:0;">
    </div>
    @error("activity")
